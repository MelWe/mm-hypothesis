function [ idimsigstats,SSVs] = variancebaseddim2struc( data,radii,K,varthreshhold,epszero,idimfigs,fname )
%This function has a similar functionality to variancebaseddim and
%variancebaseddimstruc. However, a significance check is added when
%computing idim. Only neighborhoods for which #pts >=
%localidim*log(localidim) are considered when computing the intrinsic
%dimension. 
...
[idim, idimstats] = idimprob2( CumG,q,groupdim,t,indG,ballcount);
u = unique(idim);
...
firstscaleindex = idimstats(1).firstscaleindex;
[istats,irhs,ilhs] = unique([idim,firstscaleindex],'rows');

%istatssummary = zeros(length(istats),4);
istatssummary = zeros(length(istats),3);

idimstats.idimsummary = idimsummary;
idimstats.istatssummary = istatssummary;
idimstats.ballcount = ballcount;
idimsigstats = idimstats;
